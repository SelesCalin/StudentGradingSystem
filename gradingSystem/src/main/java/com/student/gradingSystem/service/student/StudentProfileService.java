...
public class StudentProfileService {
    ...
    public StudentProfileDTO getStudentProfile(Integer id) throws CustomException{
        Optional<User> user = userRepository.findById(id);
        if(!user.isPresent()) {
            throw new CustomException("User not found.Something went wrong");
        } else {
            return new StudentProfileDTO(user.get());
        }
    }
    ...
    @Transactional
    public void updateGrupa(Integer id, Integer newGroup) throws CustomException{
        Optional<User> user = userRepository.findById(id);
        if (!user.isPresent()) {
            throw new CustomException("User not found.Something went wrong");
        }
        user.get().setGrupa(newGroup);
    }
}
